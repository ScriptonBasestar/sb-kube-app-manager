...
def test_deploy_on_test_cluster():
    ...
    assert (
        run_cmd(
            [
                ...
                "prepare",
                "--apps",
                str(EXAMPLES_DIR / "config-browserless"),
                "--sources",
                str(EXAMPLES_DIR / "sources"),
            ],
            env=env,
        ).returncode
        == 0
    ...
    assert (
        run_cmd(
            ["sbkube", "build", "--apps", str(EXAMPLES_DIR / "config-browserless")],
            env=env,
        ).returncode
        == 0
    )
...
def test_deploy_helm_app_install(
    ...
    with (
        ...
        if values_file.exists():
            ...
            any(
                "--values" in str(arg) or str(values_file) in str(arg)
                for arg in helm_cmd
            )
...
@patch(CLI_TOOLS_CHECK_PATH, return_value=None)
def test_deploy_helm_app_upgrade(
    mock_cli_tools_check,
    runner: CliRunner,
    create_sample_config_yaml,
    create_sample_values_file,
    ...
    with (
        ...
        patch(
            ...
            return_value={"my-helm-app": {"name": "my-helm-app"}},
        ),
    ):  # 이미 설치됨
        ...
        assert (
            "이미 설치되어 있습니다" in result.output or "건너뜁니다" in result.output
        )


@patch(CLI_TOOLS_CHECK_PATH, return_value=None)
def test_deploy_kubectl_app(
    mock_cli_tools_check,
    runner: CliRunner,
    create_sample_config_yaml,
...
def test_deploy_action_app(
    ...
    with patch("subprocess.run") as mock_subprocess:
        ...
        # exec 명령이 호출되었는지 확인
        exec_calls = [
            call for call in mock_subprocess.call_args_list if "echo" in str(call[0][0])
        ]
        assert len(exec_calls) > 0, "exec 명령이 호출되지 않았습니다"


def test_deploy_specific_app_with_namespace_override(
    runner: CliRunner,
    create_sample_config_yaml,
    create_sample_values_file,
    base_dir,
