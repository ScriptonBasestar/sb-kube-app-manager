...
from sbkube.utils.diagnostic_system import DiagnosticEngine, DiagnosticCheck, DiagnosticResult, DiagnosticLevel
...
class ValidationResult:
    ...
    @classmethod
    def from_diagnostic_result(cls, diagnostic_result: DiagnosticResult, 
...
class ValidationCheck(DiagnosticCheck):
    ...
    def create_validation_result(self, level: DiagnosticLevel, message: str,
                               severity: ValidationSeverity = ValidationSeverity.MEDIUM,
                               details: str = "", recommendation: str = None,
                               fix_command: str = None, fix_description: str = None,
                               risk_level: str = "low",
...
class ValidationReport:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        for result in self.results:
            ...
            # 배포 준비 상태 평가
            if result.level == DiagnosticLevel.ERROR or result.severity == ValidationSeverity.CRITICAL:
    ...
    def get_critical_results(self) -> List[ValidationResult]:
        ...
        return [r for r in self.results 
                if r.level == DiagnosticLevel.ERROR or 
                   r.severity in [ValidationSeverity.CRITICAL, ValidationSeverity.HIGH]]
...
class ValidationEngine(DiagnosticEngine):
    ...
    def __init__(self, console: Console = None, validation_mode: ValidationMode = ValidationMode.BASIC):
    ...
    async def run_validation_suite(self, context: ValidationContext, 
                                 show_progress: bool = True) -> ValidationReport:
        """검증 스위트 실행"""
        # 새 보고서 생성
        ...
        if show_progress:
            with Progress(
                ...
                for validator in self.validators:
                    progress.update(task, description=f"검증 중: {validator.description}")
                    try:
        ...
        self.results = [self._validation_to_diagnostic_result(r) for r in self.current_report.results]
        
        return self.current_report
    
    def _validation_to_diagnostic_result(self, validation_result: ValidationResult) -> DiagnosticResult:
    ...
    def display_validation_results(self, detailed: bool = False):
        ...
        # 카테고리별 결과 표시
        if detailed or summary['by_level']['error'] > 0 or summary['by_level']['warning'] > 0:
    ...
    def _display_detailed_validation_results(self):
        """상세 검증 결과 표시"""
        categories = set(r.category for r in self.current_report.results)
        
        for category in sorted(categories):
            category_results = self.current_report.get_results_by_category(category)
            error_results = [r for r in category_results if r.level == DiagnosticLevel.ERROR]
            warning_results = [r for r in category_results if r.level == DiagnosticLevel.WARNING]
    ...
    def save_report(self, file_path: Union[str, Path], format: str = "json") -> bool:
        """보고서 저장"""
        if not self.current_report:
        ...
        try:
            ...
            if format.lower() == "json":
                with open(report_path, 'w', encoding='utf-8') as f:
                    json.dump(self.current_report.to_dict(), f, 
