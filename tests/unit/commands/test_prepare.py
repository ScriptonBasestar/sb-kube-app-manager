...
def test_prepare_command_runs_successfully():
    ...
    result = subprocess.run(
        [
            ...
            "--sources-file",
            "sources.yaml",  # k3scode 루트의 sources.yaml (base-dir 기준)
        ],
        capture_output=True,
        text=True,
    )
...
def test_prepare_helm_repo_add_and_update(
    mock_cli_tools_check,
    runner: CliRunner,
    create_sample_config_yaml,
    create_sample_sources_yaml,
    base_dir,
    app_dir,
    caplog,
):
    ...
    with patch("subprocess.run") as mock_subprocess_run:
        ...
        result = runner.invoke(
            ...
            [
                ...
                "--sources-file",
                f"{app_dir.name}/{sources_file.name}",  # sources.yaml 파일 경로 추가
                "--app",
                app_name,
            ],
        )
    ...
    with patch("subprocess.run") as mock_subprocess_run:
        ...
        result_clone = runner.invoke(
            ...
            [
                ...
                str(app_dir.name),
                "--config-file",
                config_file.name,  # 파일 이름만 전달
                "--sources-file",
                f"{app_dir.name}/{sources_file.name}",  # base_dir 기준 상대 경로
                "--app",
                "my-pull-git-app",
            ],
        )
...
@patch(CLI_TOOLS_CHECK_PATH, return_value=None)
def test_prepare_no_pull_apps(
    mock_cli_tools_check, runner: CliRunner, base_dir, app_dir, caplog
):
    """
    ...
    sources_content = {"helm_repos": {"bitnami": "https://charts.bitnami.com/bitnami"}}
