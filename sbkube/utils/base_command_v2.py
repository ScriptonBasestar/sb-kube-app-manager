...
class EnhancedBaseCommand:
    ...
    def find_config_file(self) -> Path:
        ...
        if self.config_file_name:
            ...
            logger.error(f"App config file not found: {self.app_config_dir}/config.[yaml|yml|toml]")
            raise click.Abort()
    ...
    def parse_apps(
        ...
        for app_info in self.app_info_list:
            try:
                ...
                if self.validate_on_load:
                    validated_specs = app_info.get_validated_specs()
    ...
    def process_apps_with_stats(
        ...
        for app_info in self.app_info_list:
            try:
                ...
                logger.error(f"Unexpected error {operation_name} app '{app_info.name}': {e}")
                failed_apps.append(app_info.name)
