...
2. **단계별 실행 제어**: 특정 단계부터 시작 또는 특정 단계까지만 실행
3. **실패 처리**: 실패 시 중단점 저장 및 재시작 지원
4. **진행 상황 표시**: 각 단계별 실행 상태 및 진행률 표시
5. **기존 명령어 호환성**: 모든 기존 옵션과 완전 호환

### 고급 요구사항
1. **병렬 처리**: 가능한 작업들의 병렬 실행
2. **검증 및 의존성 확인**: 각 단계 실행 전 사전 검증
3. **상태 관리**: 실행 상태 저장 및 복원
4. **사용자 상호작용**: 오류 발생 시 사용자 선택 옵션 제공
