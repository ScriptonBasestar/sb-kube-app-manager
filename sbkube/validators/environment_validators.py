        ...
        try:
            ...
            for node in nodes:
                ...
                if is_ready:
                    ...
                    memory_capacity = self._parse_memory_resource(capacity.get('memory', '0'))
                    cpu_alloc = self._parse_cpu_resource(allocatable.get('cpu', '0'))
                    memory_alloc = self._parse_memory_resource(allocatable.get('memory', '0'))
            ...
            if ready_nodes == 0:
                ...
                issues.append(f"일부 노드가 NotReady 상태입니다 ({ready_nodes}/{len(nodes)} 노드만 Ready)")
            
            # 리소스 충분성 검증
            if allocatable_cpu < 1.0:
                issues.append(f"사용 가능한 CPU가 부족합니다 ({allocatable_cpu:.2f} cores)")
            elif allocatable_cpu < 2.0:
                issues.append(f"CPU 리소스가 제한적입니다 ({allocatable_cpu:.2f} cores). 2 cores 이상 권장")
            
            if allocatable_memory < 2 * 1024 * 1024 * 1024:  # 2GB
                issues.append(f"사용 가능한 메모리가 부족합니다 ({allocatable_memory / (1024**3):.2f}GB)")
            elif allocatable_memory < 4 * 1024 * 1024 * 1024:  # 4GB
                issues.append(f"메모리 리소스가 제한적입니다 ({allocatable_memory / (1024**3):.2f}GB). 4GB 이상 권장")
                    ...
                    for quota in quotas:
                        ...
                        if 'requests.cpu' in hard and 'requests.cpu' in used:
                            ...
                            if used_cpu / hard_cpu > 0.9:
                                issues.append(f"네임스페이스 '{namespace}' CPU 쿼터 거의 소진 ({used_cpu:.2f}/{hard_cpu:.2f} cores)")
                        
                        # 메모리 쿼터 확인
                        if 'requests.memory' in hard and 'requests.memory' in used:
                            hard_memory = self._parse_memory_resource(hard['requests.memory'])
                            used_memory = self._parse_memory_resource(used['requests.memory'])
                            
                            if used_memory / hard_memory > 0.9:
                                issues.append(f"네임스페이스 '{namespace}' 메모리 쿼터 거의 소진 ({used_memory / (1024**3):.2f}/{hard_memory / (1024**3):.2f}GB)")
    ...
    async def _check_storage_classes(self) -> List[str]:
        ...
        try:
            ...
            if not storage_classes:
                warnings.append("스토리지 클래스가 정의되지 않았습니다. PV/PVC 사용 시 문제가 발생할 수 있습니다")
            else:
                default_storage = any(
                    sc.get('metadata', {}).get('annotations', {}).get('storageclass.kubernetes.io/is-default-class') == 'true'
    ...
    async def _check_namespace_access(self, namespace: str) -> List[str]:
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                if create_result.returncode != 0 or "no" in create_result.stdout.lower():
                    issues.append(f"네임스페이스 '{namespace}'가 존재하지 않으며 생성 권한도 없습니다")
                else:
                    # 네임스페이스 생성 가능하다면 경고만
                    pass
            ...
            if result.returncode != 0 and "forbidden" in result.stderr.lower():
                issues.append(f"네임스페이스 '{namespace}' 내 리소스 조회 권한이 없습니다")
        ...
        for action, resource in required_permissions:
            try:
                ...
                if result.returncode != 0 or "no" in result.stdout.lower():
                    issues.append(f"권한 부족: {action} {resource} (네임스페이스: {namespace})")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if len(service_accounts) <= 1:  # default SA만 있는 경우
                    warnings.append(f"네임스페이스 '{namespace}'에 사용자 정의 ServiceAccount가 없습니다. 보안 강화를 위해 고려해보세요")
...
class NetworkPolicyValidator(ValidationCheck):
        ...
        for name, url, timeout in test_endpoints:
            try:
                ...
                if response.status_code >= 500:
                    ...
                    logger.debug(f"{name} 인증 필요 또는 접근 제한 (HTTP {response.status_code})")
    ...
    async def _check_network_policies(self, context: ValidationContext) -> List[str]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if not policies:
                    warnings.append(f"네임스페이스 '{namespace}'에 네트워크 정책이 설정되지 않았습니다. 보안 강화를 위해 고려해보세요")
                else:
                    # 정책이 너무 제한적인지 확인
                    ...
                    if restrictive_policies:
                        warnings.append(f"일부 네트워크 정책이 매우 제한적입니다: {', '.join(restrictive_policies)}")
    ...
    async def _check_ingress_setup(self) -> List[str]:
        ...
        try:
            # Ingress 컨트롤러 확인
            result = subprocess.run([
                "kubectl", "get", "pods", "-A", "-l", "app.kubernetes.io/name=ingress-nginx", "-o", "json"
            ], capture_output=True, text=True, timeout=10)
            ...
            if result.returncode == 0:
                ...
                if not ingress_pods:
                    warnings.append("Ingress 컨트롤러가 설치되지 않았습니다. 외부 접근이 필요한 경우 설치를 고려해보세요")
                else:
                    # Ingress 컨트롤러 상태 확인
                    ready_pods = 0
                    ...
                    if ready_pods == 0:
                        warnings.append("Ingress 컨트롤러 파드가 모두 비정상 상태입니다")
                    elif ready_pods < len(ingress_pods):
                        warnings.append(f"일부 Ingress 컨트롤러 파드가 비정상 상태입니다 ({ready_pods}/{len(ingress_pods)})")
...
class SecurityContextValidator(ValidationCheck):
    ...
    async def _check_pod_security_standards(self, context: ValidationContext) -> List[str]:
        """Pod Security Standards 확인"""
        warnings = []
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if missing_pss:
                    warnings.append(f"네임스페이스 '{namespace}'에 Pod Security Standards 레이블이 설정되지 않았습니다")
                else:
                    # 보안 수준 확인
                    enforce_level = labels.get('pod-security.kubernetes.io/enforce', '')
                    if enforce_level == 'privileged':
                        warnings.append(f"네임스페이스 '{namespace}'의 Pod Security 수준이 'privileged'로 설정되어 보안이 약합니다")
    ...
    async def _check_security_policies(self) -> List[str]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if policies:
                    warnings.append("Pod Security Policy가 설정되어 있습니다. Kubernetes 1.25+에서는 deprecated되었으니 Pod Security Standards로 마이그레이션을 고려하세요")
