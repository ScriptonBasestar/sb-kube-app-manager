...
class MissingNamespaceFix(AutoFix):
    ...
    def can_fix(self, diagnostic_result: DiagnosticResult) -> bool:
        ...
        return (
            ...
            ("존재하지 않음" in diagnostic_result.message or "존재하지 않습니다" in diagnostic_result.message)
        )
...
class ConfigFileFix(AutoFix):
    ...
    def can_fix(self, diagnostic_result: DiagnosticResult) -> bool:
        ...
        return (
            ...
            "config.yaml" in diagnostic_result.message or
            "필수 설정이 누락" in diagnostic_result.message or
            "YAML 문법 오류" in diagnostic_result.message
        )
...
class HelmRepositoryFix(AutoFix):
    ...
    def can_fix(self, diagnostic_result: DiagnosticResult) -> bool:
        ...
        return (
            ...
            ("리포지토리" in diagnostic_result.message or "repository" in diagnostic_result.message.lower())
        )
            ...
            for name, url in repositories:
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        logger.warning(f"Helm 리포지토리 추가 실패: {name} - {result.stderr}")
