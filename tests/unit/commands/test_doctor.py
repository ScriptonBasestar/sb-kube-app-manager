...
class TestDiagnosticEngine:
    ...
    async def test_run_all_checks(self):
        ...
        mock_result1 = DiagnosticResult("check1", DiagnosticLevel.SUCCESS, "성공")
        ...
        mock_result2 = DiagnosticResult("check2", DiagnosticLevel.ERROR, "실패")
    ...
    def test_get_summary(self):
        ...
        engine.results = [
            ...
            DiagnosticResult(
                "check2", DiagnosticLevel.ERROR, "실패", fix_command="fix"
            ),
    ...
    def test_get_fixable_results(self):
        ...
        non_fixable = DiagnosticResult("check2", DiagnosticLevel.ERROR, "실패")
    ...
    def test_has_errors_and_warnings(self):
        ...
        # 오류 없음
        engine.results = [DiagnosticResult("check1", DiagnosticLevel.SUCCESS, "성공")]
        ...
        # 오류 있음
        engine.results.append(DiagnosticResult("check3", DiagnosticLevel.ERROR, "오류"))
...
class TestDiagnosticResult:
    ...
    def test_is_fixable(self):
        """수정 가능성 테스트"""
        fixable = DiagnosticResult(
            "test", DiagnosticLevel.ERROR, "메시지", fix_command="fix"
        )
...
class TestResourceAvailabilityCheck:
    ...
    async def test_no_ready_nodes(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0, stdout="node1   NotReady\nnode2   NotReady"
            )
    ...
    async def test_low_disk_space(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(returncode=0, stdout="node1   Ready")
    ...
    async def test_resources_ok(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0, stdout="node1   Ready\nnode2   Ready"
            )
