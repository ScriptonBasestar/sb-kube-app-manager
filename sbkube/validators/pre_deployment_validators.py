...
class DeploymentSimulator(ValidationCheck):
    ...
    async def run_validation(self, context: ValidationContext) -> ValidationResult:
        ...
        try:
            ...
            for app_name, app_config in apps:
                app_results = await self._simulate_app_deployment(app_name, app_config, context)
            ...
            # 전체 배포 시뮬레이션 요약
            total_resources = sum(result.get('resource_count', 0) for result in simulation_results)
    ...
    async def _get_deployment_apps(self, context: ValidationContext) -> List[Tuple[str, Dict[str, Any]]]:
        """배포할 앱들 추출"""
        apps = []
    ...
    async def _simulate_namespace_creation(self, context: ValidationContext) -> List[str]:
        """네임스페이스 생성 시뮬레이션"""
        issues = []
                ...
                if namespace != 'default':
                    ...
                    if result.returncode != 0:
                        ...
                        if create_result.returncode != 0:
                            issues.append(f"네임스페이스 '{namespace}' 생성 시뮬레이션 실패: {create_result.stderr.strip()}")
    ...
    async def _simulate_app_deployment(self, app_name: str, app_config: Dict[str, Any], context: ValidationContext) -> Dict[str, Any]:
        """개별 앱 배포 시뮬레이션"""
        result = {
            'app_name': app_name,
    ...
    async def _simulate_helm_deployment(self, app_name: str, specs: Dict[str, Any], context: ValidationContext, result: Dict[str, Any]):
        """Helm 앱 배포 시뮬레이션"""
        base_path = Path(context.base_dir)
        ...
        if not chart_path.exists():
            result['issues'].append(f"앱 '{app_name}': 차트 경로 '{chart_path_str}'가 존재하지 않습니다")
            return
        ...
        try:
            ...
            if result_proc.returncode != 0:
                result['issues'].append(f"앱 '{app_name}': Helm 템플릿 렌더링 실패 - {result_proc.stderr.strip()}")
                return
    ...
    async def _simulate_yaml_deployment(self, app_name: str, specs: Dict[str, Any], context: ValidationContext, result: Dict[str, Any]):
        """YAML 앱 배포 시뮬레이션"""
        base_path = Path(context.base_dir)
        ...
        for action in actions:
            ...
            if action_type == 'apply' and action_path:
                ...
                if not yaml_path.exists():
                    result['issues'].append(f"앱 '{app_name}': YAML 파일 '{action_path}'이 존재하지 않습니다")
                    continue
                ...
                try:
                    ...
                    result['issues'].append(f"앱 '{app_name}': YAML 파일 '{action_path}' 읽기 실패 - {e}")
    ...
    async def _analyze_rendered_resources(self, yaml_content: str) -> List[Dict[str, Any]]:
        """렌더링된 YAML에서 리소스 분석"""
        resources = []
    ...
    async def _test_kubectl_dry_run(self, yaml_content: str, namespace: str) -> List[str]:
        ...
        try:
            with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False) as temp_file:
                ...
                result = subprocess.run([
                    ...
                    "--dry-run=server", "--namespace", namespace
...
class RiskAssessmentValidator(ValidationCheck):
    ...
    async def run_validation(self, context: ValidationContext) -> ValidationResult:
        ...
        return self.create_validation_result(
            ...
            details="위험 요소 분석:\n" + "\n".join(f"• {factor}" for factor in risk_factors) if risk_factors else "식별된 위험 요소가 없습니다.",
    ...
    async def _assess_resource_impact(self, context: ValidationContext) -> Dict[str, Any]:
        """리소스 영향도 평가"""
        factors = []
        score = 0
    ...
    async def _assess_network_impact(self, context: ValidationContext) -> Dict[str, Any]:
        """네트워크 정책 위험도 평가"""
        factors = []
        score = 0
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if ingresses:
                    factors.append(f"외부 노출 서비스 존재 (Ingress: {len(ingresses)}개)")
    ...
    async def _assess_persistence_impact(self, context: ValidationContext) -> Dict[str, Any]:
        """데이터 지속성 위험도 평가"""
        factors = []
        score = 0
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if pvcs:
                    ...
                    factors.append(f"영구 스토리지 사용 (PVC: {len(pvcs)}개, 총 {total_storage:.1f}GB)")
    ...
    async def _assess_security_impact(self, context: ValidationContext) -> Dict[str, Any]:
        """보안 위험도 평가"""
        factors = []
        score = 0
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                custom_sa_count = len([sa for sa in service_accounts if sa.get('metadata', {}).get('name') != 'default'])
                
                if custom_sa_count > 0:
                    factors.append(f"사용자 정의 ServiceAccount 사용 ({custom_sa_count}개)")
    ...
    def _has_privileged_settings(self, values_data: Dict[str, Any]) -> bool:
        ...
        # 일반적인 높은 권한 설정 패턴들
        privileged_patterns = [
            'privileged', 'runAsRoot', 'allowPrivilegeEscalation',
        ...
        def check_nested_dict(data):
            if isinstance(data, dict):
                for key, value in data.items():
                    if any(pattern in str(key).lower() for pattern in privileged_patterns):
        ...
        return check_nested_dict(values_data)
    ...
    def _get_risk_recommendation(self, risk_level: str, risk_factors: List[str]) -> str:
        ...
        if risk_level == "CRITICAL":
            ...
            return "위험 요소를 검토하고 배포 후 모니터링을 통해 시스템 상태를 확인하세요."
        else:
...
class RollbackPlanValidator(ValidationCheck):
            ...
            if result.returncode == 0:
                ...
                for release in releases:
                    ...
                    if history_result.returncode == 0:
                        ...
                        if len(history) < 2:
                            plan['helm_issues'].append(f"릴리스 '{release_name}': 롤백할 이전 버전이 없습니다")
            else:
    ...
    async def _assess_backup_capability(self, context: ValidationContext) -> Dict[str, Any]:
        """백업 가능성 평가"""
        plan = {
            'backup_tools': [],
    ...
    async def _assess_persistence_rollback(self, context: ValidationContext) -> Dict[str, Any]:
        """데이터 영속성 롤백 평가"""
        plan = {
            'persistent_volumes': [],
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if len(pvcs) > 0:
                    ...
                    plan['persistence_issues'].append(f"영구 볼륨 데이터 롤백 위험 ({len(pvcs)}개 PVC)")
    ...
    def _evaluate_overall_rollback(self, rollback_plan: Dict[str, Any]) -> Dict[str, Any]:
        """종합 롤백 가능성 평가"""
        critical_issues = []
        warnings = []
...
class ImpactAnalysisValidator(ValidationCheck):
    ...
    async def _analyze_namespace_impact(self, context: ValidationContext) -> Dict[str, Any]:
        """네임스페이스 충돌 분석"""
        analysis = {
            'namespace_conflicts': [],
            'existing_workloads': 0
        }
    ...
    async def _analyze_resource_conflicts(self, context: ValidationContext) -> Dict[str, Any]:
        """리소스 이름 충돌 분석"""
        analysis = {
            'resource_conflicts': [],
            'potential_conflicts': []
    ...
    async def _analyze_service_conflicts(self, context: ValidationContext) -> Dict[str, Any]:
        """서비스 및 포트 충돌 분석"""
        analysis = {
            'service_conflicts': [],
            'port_conflicts': []
    ...
    async def _analyze_resource_usage_impact(self, context: ValidationContext) -> Dict[str, Any]:
        """리소스 사용량 영향 분석"""
        analysis = {
            'resource_pressure': [],
            'usage_warnings': []
                ...
                for line in node_lines:
                    if line.strip():
                        ...
                        if len(parts) >= 5:
                            ...
                            cpu_percent = int(cpu_usage.rstrip('%')) if cpu_usage.endswith('%') else 0
                            memory_percent = int(memory_usage.rstrip('%')) if memory_usage.endswith('%') else 0
                            
                            if cpu_percent > 80 or memory_percent > 80:
                                high_usage_nodes.append(f"{node_name} (CPU: {cpu_percent}%, Memory: {memory_percent}%)")
    ...
    async def _get_new_resource_names(self, context: ValidationContext) -> Dict[str, List[str]]:
        """배포될 새 리소스 이름들 추출"""
        resource_names = {
            'deployments': [],
            'services': [],
    ...
    def _evaluate_overall_impact(self, impact_analysis: Dict[str, Any]) -> Dict[str, Any]:
        """종합 영향도 평가"""
        critical_issues = []
        warnings = []
