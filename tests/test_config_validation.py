...
from sbkube.models.base_model import ConfigBaseModel, InheritableConfigModel
from sbkube.models.config_model_v2 import (
    AppInfoScheme, AppGroupScheme, AppInstallHelmSpec,
    AppPullGitSpec, AppPullHelmSpec, CopyPair
)
from sbkube.models.sources_model_v2 import SourceScheme, GitRepoScheme, HelmRepoScheme
...
class TestValidators:
    ...
    def test_url_validation(self):
        """Test URL validation."""
        # Valid URLs
        assert ValidatorMixin.validate_url("https://example.com", ["https"]) == "https://example.com"
        assert ValidatorMixin.validate_url("http://localhost:8080", ["http", "https"]) == "http://localhost:8080"
...
class TestConfigModels:
        ...
        with pytest.raises(ConfigValidationError, match="Duplicate app names"):
            AppGroupScheme(
                ...
                apps=[
                    ...
                    AppInfoScheme(name="app1", type="exec", specs={"commands": ["pwd"]})
                ]
            )
    ...
    def test_git_spec_validation(self):
        ...
        spec = AppPullGitSpec(
            repo="my-repo",
            paths=[
                CopyPair(src="src/app", dest="dest/app")
            ]
        )
...
class TestSourcesModel:
    ...
    def test_sources_validation(self):
        ...
        with pytest.raises(ConfigValidationError, match="Git URL must start with"):
            SourceScheme(
                cluster="cluster",
                git_repos={
                    "bad": GitRepoScheme(url="invalid-url", branch="main")
                }
            )
    ...
    def test_helm_repo_validation(self):
        ...
        # Invalid TLS config
        with pytest.raises(ConfigValidationError, match="all three files must be specified"):
            HelmRepoScheme(
                url="https://charts.example.com",
                ca_file="/path/ca.crt"
            )
    ...
    def test_git_repo_validation(self):
        ...
        with pytest.raises(ConfigValidationError, match="Only one authentication method"):
            GitRepoScheme(
                url="https://github.com/example/repo.git",
                branch="main",
                username="user",
