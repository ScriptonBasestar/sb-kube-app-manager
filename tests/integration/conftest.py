...
def k8s_cluster():
    ...
    with K3sContainer() as k3s:
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False) as f:
            ...
            kubeconfig_data['clusters'][0]['cluster']['server'] = k3s.get_connection_url()
...
"""
    ...
    # Package the chart
    subprocess.run(["helm", "package", str(test_chart_dir), "-d", str(charts_dir)], check=True)
...
def performance_tracker():
    ...
    def get_summary():
        ...
        return {
            ...
            'slowest_operation': max(metrics['operations'], key=lambda x: x['duration']) if metrics['operations'] else None
        }
