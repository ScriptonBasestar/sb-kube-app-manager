    ...
    def test_run_with_profile(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch('sbkube.commands.run.ProfileManager') as mock_pm_class:
                ...
                # 실제 단계 실행을 모킹
                with patch.object(RunCommand, '_execute_step') as mock_execute:
                    cmd = RunCommand(
                        base_dir=tmpdir,
                    ...
                    mock_pm_class.assert_called_once_with(Path(tmpdir), Path(tmpdir) / "config")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch('sbkube.commands.run.ProfileManager') as mock_pm_class:
                ...
                with patch('sbkube.utils.logger.logger') as mock_logger:
                    ...
                    # 경고 메시지가 출력되었는지 확인
                    mock_logger.warning.assert_called_with("⚠️  프로파일 경고: no apps defined")
