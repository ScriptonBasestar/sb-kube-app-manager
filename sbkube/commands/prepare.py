...
from sbkube.models.config_model import (AppInfoScheme, AppPullGitSpec,
                                        AppPullHelmOciSpec, AppPullHelmSpec)
...
def cmd(
    ...
    if needed_helm_repo_names:
        return_code, stdout, stderr = run_command(
            ["helm", "repo", "list", "-o", "json"], timeout=10
        )
        if return_code == 0:
            try:
        ...
        for repo_name in needed_helm_repo_names:
            ...
            if needs_update:
                ...
                return_code, _, stderr = run_command(
                    ["helm", "repo", "update", repo_name], check=False, timeout=60
                )
                if return_code == 0:
                    console.print(
    ...
    for app_info in app_info_list:
        ...
        return_code, stdout, stderr = run_command(
            helm_pull_cmd, check=False, timeout=300
        )
