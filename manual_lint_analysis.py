...
def analyze_file(file_path: Path) -> Dict[str, List[str]]:
    ...
    for i, line in enumerate(lines, 1):
        ...
        if len(line.rstrip()) > 88:
            issues['style_issues'].append(f"Line {i}: Line too long ({len(line.rstrip())} > 88)")
        ...
        if 'print(' in line_stripped and 'logger' not in ''.join(lines[:i]):
            issues['potential_bugs'].append(f"Line {i}: Direct print() usage (should use logger)")
        
        # Type issues
        if re.search(r'def \w+\([^)]*\):', line_stripped) and '->' not in line_stripped:
            if not line_stripped.startswith('def __'):  # Skip dunder methods
                issues['type_issues'].append(f"Line {i}: Function missing return type annotation")
    ...
    if 'class ' in content:
        ...
        for match in class_matches:
            ...
            if '"""' not in content[match.end():match.end()+200]:
                issues['docstring_issues'].append(f"Class {class_name} missing docstring")
...
def main():
    ...
    print(f"Analysis complete. Report saved to: lint-output.txt")
