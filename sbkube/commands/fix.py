...
from sbkube.diagnostics.kubernetes_checks import (
    ...
    NetworkAccessCheck, PermissionsCheck, ResourceAvailabilityCheck
)
...
def cmd(ctx, dry_run, force, rollback, backup_cleanup, history, config_dir):
    ...
    try:
        ...
        results = await asyncio.create_task(diagnostic_engine.run_all_checks(show_progress=False))
...
def _show_fix_history(fix_engine: AutoFixEngine) -> None:
    ...
    if rollback_candidates:
        ...
        for i, attempt in enumerate(rollback_candidates, 1):
            console.print(f"  {i}. {attempt.description} ({attempt.timestamp.strftime('%Y-%m-%d %H:%M:%S')})")
        
        console.print(f"\n💡 [bold]sbkube fix --rollback N[/bold] 명령어로 최근 N개 수정을 롤백할 수 있습니다.")


async def run_fix_command(ctx, dry_run, force, rollback, backup_cleanup, history, config_dir):
    """비동기 실행을 위한 래퍼 함수"""
    return await cmd.callback(ctx, dry_run, force, rollback, backup_cleanup, history, config_dir)
