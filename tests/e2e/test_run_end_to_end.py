...
class TestRunEndToEnd:
    ...
    def test_run_option_validation_e2e(self):
        ...
        with runner.isolated_filesystem():
            ...
            invalid_combinations = [
                ...
                ['run', '--only', 'template', '--from-step', 'build', '--to-step', 'deploy'],
            ]
    ...
    def test_complete_workflow_simulation(self):
        ...
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(main, ['run', '--from-step', 'build', '--to-step', 'template', '--dry-run'])
            assert result.exit_code == 0
...
""")
    ...
    def test_run_command_integration_completeness(self):
        ...
        required_options = ['--from-step', '--to-step', '--only', '--dry-run', '--app-dir', '--base-dir', '--app', '--config-file']
        for option in required_options:
            assert option in help_output, f"Missing option: {option}"
        
        # 도움말 섹션 존재 확인
        required_sections = ['기본 사용법:', '단계별 실행 제어:', '환경 설정:', '문제 해결:']
