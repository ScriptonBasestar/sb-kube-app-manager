...
# Run ruff with fixes
run_cmd("uv run ruff check sbkube tests --fix --unsafe-fixes --exclude migrations --exclude node_modules --exclude examples", 
        "Running ruff with auto-fix and unsafe-fixes")

# Run ruff format
run_cmd("uv run ruff format sbkube tests --exclude migrations --exclude node_modules --exclude examples", 
        "Running ruff format")

# Run mdformat
run_cmd("uv run mdformat *.md docs/**/*.md --wrap 120", 
        "Running mdformat on markdown files")
