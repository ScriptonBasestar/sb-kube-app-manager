...
from sbkube.models.deployment_state import (
    Base, Deployment, AppDeployment, DeployedResource, HelmRelease,
    DeploymentStatus, ResourceAction, DeploymentCreate, AppDeploymentCreate,
    ResourceInfo, HelmReleaseInfo, DeploymentSummary, DeploymentDetail
)
...
class DeploymentDatabase:
        ...
        with self.get_session() as session:
            ...
            for app_dep in deployment.app_deployments:
                ...
                for resource in app_dep.resources:
                    detail.resources.append(ResourceInfo(
                ...
                # Add Helm releases
                helm_releases = session.query(HelmRelease).filter_by(
                    app_deployment_id=app_dep.id
                ).all()
                
                for release in helm_releases:
                    detail.helm_releases.append(HelmReleaseInfo(
    ...
    def list_deployments(
        ...
        with self.get_session() as session:
            ...
            for deployment in query:
                ...
                success_apps = sum(
                    1 for app in deployment.app_deployments
    ...
    def cleanup_old_deployments(
        ...
        with self.get_session() as session:
            ...
            cutoff_date = text(
                f"datetime('now', '-{days_to_keep} days')"
            )
            
            old_deployments = session.query(Deployment).filter(
                Deployment.timestamp < cutoff_date
            ).all()
