...
Helm 타입은 기본적으로 `helm upgrade --install --wait --timeout 5m` 형태로 실행됩니다. 앱 단위로 다음 필드를 조정해 동작을 세밀하게 제어할 수 있습니다.
...
> **주의** `depends_on`은 배포 순서만 보장합니다. `wait: false`로 설정한 앱이 종속된 리소스가 준비되기 전에 다음 앱으로 넘어갈 수 있으므로, 필요하면
> `kubectl rollout status`나 `exec` 앱을 활용해 상태를 직접 검증하세요.
