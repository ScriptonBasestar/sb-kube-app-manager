...
def run_ruff_check() -> Dict[str, str]:
    ...
    cmd_strict = [
        ...
        "sbkube", "tests",
        "--select", "ALL",
        "--ignore", "E501,B008,C901,COM812,B904,B017,B007,D100,D101,D102,D103,D104,D105,D106,D107",
        "--exclude", "migrations",
        "--exclude", "node_modules",
        "--exclude", "examples",
        "--output-format", "full"
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                if "import *" in content:
                    issues["import_issues"].append(f"{file_path}: Contains wildcard imports")
                
                # Check for syntax (basic)
                try:
                    compile(content, str(file_path), 'exec')
                except SyntaxError as e:
                    issues["syntax_issues"].append(f"{file_path}: Syntax error at line {e.lineno}: {e.msg}")
...
def main():
    ...
    try:
        ...
        print(f"Report saved to: lint-output.txt")
