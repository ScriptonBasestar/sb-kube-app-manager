...
1. [제품 개요](#1-제품-개요)
2. [왜 만드는가 (문제 정의)](#2-왜-만드는가-문제-정의)
3. [누구를 위한가 (대상 사용자)](#3-누구를-위한가-대상-사용자)
4. [무엇을 해결하는가 (가치 제안)](#4-무엇을-해결하는가-가치-제안)
5. [어떻게 사용하는가 (사용자 시나리오)](#5-어떻게-사용하는가-사용자-시나리오)
6. [주요 기능](#6-주요-기능)
7. [UI/UX 구상](#7-uiux-구상)
8. [경쟁 비교](#8-경쟁-비교)
...
중소규모 DevOps 팀을 위해 Helm 차트, YAML 매니페스트, Git 리포지토리를 하나의 선언적 설정(YAML)으로 통합하여 4단계 워크플로우(`prepare → build → template → deploy`)로 자동 배포하는 Python CLI 도구
