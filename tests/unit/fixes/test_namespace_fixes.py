...
class TestMissingNamespaceFix:
    ...
    def test_extract_namespace_name(self):
        ...
        # 따옴표 테스트
        assert (
            fix._extract_namespace_name("네임스페이스 'test-ns'가 존재하지 않습니다")
            == "test-ns"
        )

        # 쌍따옴표 테스트
        assert (
            fix._extract_namespace_name('네임스페이스 "prod-ns"가 존재하지 않습니다')
            == "prod-ns"
        )

        # 단어 매칭 테스트
        assert (
            fix._extract_namespace_name("네임스페이스 dev-env 존재하지 않음")
            == "dev-env"
        )
...
class TestConfigFileFix:
    ...
    def test_add_missing_fields(
        self, mock_exists, mock_yaml_load, mock_yaml_dump, mock_open
    ):
        """누락된 필드 추가 테스트"""
        fix = ConfigFileFix()
...
class TestHelmRepositoryFix:
    ...
    def test_create_backup_success(self, mock_open, mock_run):
        ...
        mock_run.return_value = Mock(
            returncode=0,
            stdout='[{"name": "stable", "url": "https://charts.helm.sh/stable"}]',
        )
    ...
    def test_validate_fix_failure(self, mock_run, sample_helm_result):
        ...
        mock_run.return_value = Mock(returncode=0, stdout="NAME\tURL\n")
    ...
    def test_rollback_success(
        self, mock_json_loads, mock_json_load, mock_open, mock_run
    ):
        """롤백 성공 테스트"""
        fix = HelmRepositoryFix()
        ...
        mock_run.side_effect = [
            Mock(
                returncode=0, stdout='[{"name": "new-repo", "url": "https://new.repo"}]'
            ),
        ...
        mock_json_load.return_value = [
            {"name": "original-repo", "url": "https://original.repo"}
        ]
...
class TestFixesIntegration:
    ...
    def test_all_fixes_basic_interface(self):
        ...
        fixes = [MissingNamespaceFix(), ConfigFileFix(), HelmRepositoryFix()]
