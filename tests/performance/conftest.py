...
def performance_benchmark():
    ...
    def get_report() -> Dict[str, Any]:
        ...
        return {
            ...
            'average_cpu_percent': sum(b.cpu_percent for b in benchmarks) / len(benchmarks),
            'peak_memory_mb': max(b.memory_mb for b in benchmarks),
            'total_disk_read_mb': sum(b.disk_io_read for b in benchmarks) / 1024 / 1024,
            'total_disk_write_mb': sum(b.disk_io_write for b in benchmarks) / 1024 / 1024,
        }
...
def large_project_generator():
    ...
    def generate_project(
        ...
        sources = {
            ...
            "helm_repos": {
                f"repo-{i}": f"https://example.com/repo-{i}"
                for i in range(5)
            },
            "git_repos": {
                f"git-{i}": {
        ...
        for i in range(num_apps):
            ...
            if app_type == "install-helm":
                specs = {
                    ...
                    "values": [f"{app_name}-{j}.yaml" for j in range(num_values_per_app)]
                }
                ...
                specs = {
                    "paths": [{
                        "src": f"source/{app_name}",
                        "dest": f"dest/{app_name}"
                    }]
                }
