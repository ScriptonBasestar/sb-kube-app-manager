...
class TestHelmIntegration:
    ...
    def test_helm_repo_management(self, tmp_path, helm_binary):
        ...
        repo_list = subprocess.run(
            [helm_binary, "repo", "list", "-o", "json"], capture_output=True, text=True
        )
    ...
    def test_helm_chart_versioning(self, tmp_path, helm_repo_server):
        ...
        sources = {
            "cluster": "test",
            "helm_repos": {"test-repo": helm_repo_server["url"]},
        }
        with open(project_dir / "sources.yaml", "w") as f:
            yaml.dump(sources, f)
        ...
        # Add test repo
        subprocess.run(
            ["helm", "repo", "add", "test-repo", helm_repo_server["url"]], check=True
        )
    ...
    def test_helm_values_merging(self, tmp_path):
        ...
        base_values = {
            "replicaCount": 1,
            "image": {"repository": "nginx", "tag": "1.19"},
        ...
        chart_yaml = {"apiVersion": "v2", "name": "test-app", "version": "1.0.0"}
    ...
    def test_helm_deployment_lifecycle(self, tmp_path, k8s_cluster, k8s_namespace):
        ...
        with open(chart_dir / "Chart.yaml", "w") as f:
            yaml.dump(
                {"apiVersion": "v2", "name": "lifecycle-app", "version": "1.0.0"}, f
            )
            ...
            "apps": [
                {
                    ...
                    "specs": {"path": "../../charts/lifecycle-app"},
                }
            ],
        }
        with open(config_dir / "config.yaml", "w") as f:
...
class TestHelmErrorHandling:
    ...
    def test_invalid_chart_handling(self, tmp_path):
        ...
        result = runner.invoke(
            main, ["template", "--base-dir", str(project_dir), "--app-dir", "config"]
        )
        assert result.exit_code != 0
    ...
    def test_helm_repo_unreachable(self, tmp_path):
        ...
        sources = {
            "cluster": "test",
            "helm_repos": {"invalid": "http://invalid.example.com/charts"},
        }
        with open(project_dir / "sources.yaml", "w") as f:
            yaml.dump(sources, f)
