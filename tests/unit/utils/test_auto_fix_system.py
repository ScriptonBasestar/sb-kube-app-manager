...
from sbkube.utils.auto_fix_system import AutoFix, AutoFixEngine, FixAttempt, FixResult
...
class TestAutoFixEngine:
    ...
    def test_apply_fixes_success(self, fix_engine, sample_diagnostic_result):
        ...
        with patch("sbkube.utils.auto_fix_system.Path.exists", return_value=True):
            attempts = fix_engine.apply_fixes(
                [sample_diagnostic_result], interactive=False
            )
    ...
    def test_get_rollback_candidates(self, fix_engine):
        ...
        candidates = [
            FixAttempt(
                "fix1", "Fix 1", "cmd1", FixResult.SUCCESS, backup_path="/backup1"
            ),
            FixAttempt("fix2", "Fix 2", "cmd2", FixResult.FAILED),  # 실패한 것은 제외
            FixAttempt(
                "fix3", "Fix 3", "cmd3", FixResult.SUCCESS, backup_path="/backup3"
            ),
            FixAttempt(
                "fix4", "Fix 4", "cmd4", FixResult.SUCCESS
            ),  # 백업 없는 것은 제외
        ]
...
class TestMockAutoFix:
    ...
    def test_mock_auto_fix_customized(self, sample_diagnostic_result):
        ...
        fix = MockAutoFix(
            fix_id="custom_fix", description="Custom Fix", risk_level="high"
        )
