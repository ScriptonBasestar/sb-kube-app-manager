...
### 6.1 핵심 기능 (v0.11.0 안정 버전)

| 기능 | 설명 | 사용자 가치 |
|------|------|------------|
| **통합 워크플로우** | `sbkube apply` 한 번에 전체 배포 | 배포 시간 70% 절감 |
| **다중 소스 통합** | Helm, YAML, Git, HTTP, Kustomize 지원 | 도구 파편화 해소 |
| **설정 검증** | Pydantic 기반 런타임 검증 | 배포 오류 90% 사전 감지 |
| **상태 관리** | SQLAlchemy DB 기반 히스토리 추적 | 배포 추적 및 감사 |
| **Hooks 시스템** | 배포 전후 자동 스크립트 실행 | 백업/알림 자동화 |
| **앱 그룹 의존성** | `deps` 필드로 배포 순서 관리 | 의존성 기반 자동 검증 |
| **차트 충돌 방지** | `{repo}/{chart}-{version}/` 경로 구조 | 다중 버전/리포 공존 |

### 6.1.1 Workspace 기능 (v0.11.0)

| 기능 | 설명 | 상태 |
|------|------|------|
| **Workspace** | 다단계 Phase 배포 오케스트레이션 | 🟡 Preview |
| **Phase 의존성** | Kahn's algorithm 기반 위상 정렬 | ✅ 구현 완료 |
| **병렬 실행** | 독립 Phase 동시 실행 | ✅ 구현 완료 |

### 6.1.2 고도화 기능 (v0.7.0+)

| 기능 | 설명 | 상태 |
|------|------|------|
| **LLM 친화적 출력** | `--format llm/json/yaml` 옵션 | 🟡 Phase 1-3 완료 |
| **향상된 에러 처리** | 자동 에러 분류 및 수정 제안 | 🟡 개발 중 |
...
> ⚠️ **주의**: 이 섹션은 간단한 기술 개요만 제공합니다. 상세 기술 사양은 [ARCHITECTURE.md](ARCHITECTURE.md)를 참조하세요.
...
- **v0.9.0** (2025-01-25): Workspace 다단계 Phase 배포, 의존성 기반 실행 순서, 병렬 실행 지원
- **v0.8.1** (2025-11-25): Documentation as Code (`notes` 필드), 테스트 인프라 개선
- **v0.8.0** (2025-11-13): Chart path collision 방지, PV/PVC 검증
- **v0.7.0**: LLM 친화적 출력 시스템 (Phase 1-3 완료), 향상된 에러 처리
...
- **[사용법](USAGE.md)** - 빠른 사용법 참조
- **[설정 가이드](docs/03-configuration/config-schema.md)** - 설정 스키마
- **[튜토리얼](docs/08-tutorials/README.md)** - 실전 사용 예제

### 기술 문서

- **[ARCHITECTURE.md](ARCHITECTURE.md)** - 아키텍처 요약
...
**💡 기술 구현 상세는 [ARCHITECTURE.md](ARCHITECTURE.md)를 참조하세요**
