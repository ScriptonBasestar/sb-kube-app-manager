...
@click.pass_context
def cmd(ctx, app_config_dir_name: str, base_dir: str, target_app_name: str | None, dry_run: bool, skip_install: bool, config_file_name: str | None):
    ...
    for app_dict in apps_to_process:
        ...
        try:
            return_code, stdout, stderr = run_command(helm_upgrade_cmd, check=False, timeout=600)

            if return_code == 0:
                console.print(f"[green]✅ Helm 앱 '{app_release_name}' 업그레이드/설치 성공.[/green]")
                if stdout and dry_run:
                    console.print(f"    [blue]Dry-run 결과 (STDOUT):[/blue] {stdout.strip()}")
                elif stdout:
                ...
                console.print(f"[red]❌ Helm 앱 '{app_release_name}' 업그레이드/설치 실패 (exit code: {return_code}):[/red]")
