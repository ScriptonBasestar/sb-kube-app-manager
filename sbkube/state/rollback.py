...
from sbkube.models.deployment_state import (
    DeploymentStatus, ResourceAction, RollbackRequest,
    DeploymentDetail
)
...
class RollbackManager:
    ...
    def rollback_deployment(
        ...
        if not current_deployment:
            raise RollbackError(f"Deployment not found: {rollback_request.deployment_id}")
        
        # Get target deployment if specified
        target_deployment = None
        if rollback_request.target_deployment_id:
            target_deployment = self.db.get_deployment(rollback_request.target_deployment_id)
        ...
        if rollback_request.dry_run:
            ...
            return self._simulate_rollback(current_deployment, target_deployment, rollback_request)
        else:
            return self._execute_rollback(current_deployment, target_deployment, rollback_request)
    ...
    def _validate_rollback(
        ...
        if target_deployment:
            if target_deployment.cluster != current_deployment.cluster:
                raise RollbackError("Cannot rollback to deployment from different cluster")
            
            if target_deployment.timestamp >= current_deployment.timestamp:
                raise RollbackError("Target deployment is newer than current deployment")
    ...
    def _simulate_rollback(
        ...
        results = {
            ...
            "target_deployment": target_deployment.deployment_id if target_deployment else "previous",
            "actions": []
        }
        ...
        for app in current_deployment.apps:
            ...
            if app_type == "install-helm" and rollback_info.get("type") == "helm":
                ...
                app_resources = [
                    r for r in current_deployment.resources
                ...
                for resource in app_resources:
                    ...
                    elif resource.action == ResourceAction.UPDATE and resource.previous_state:
                        action = {
                            "app": app["name"],
    ...
    def _rollback_app(
        ...
        if app_type == "install-helm" and rollback_info.get("type") == "helm":
            ...
            app_resources = [
                r for r in deployment.resources
            ...
            for resource in app_resources:
                try:
                    ...
                    elif resource.action == ResourceAction.DELETE and resource.previous_state:
                    ...
                    logger.error(f"Failed to rollback resource {resource.kind}/{resource.name}: {e}")
    ...
    def _rollback_helm_release(
        ...
        logger.info(f"Rolling back Helm release: {release_name} to revision {target_revision}")
        ...
        for deployment in deployments:
            ...
            if detail and detail.app_config_dir == app_config_dir:
                rollback_points.append({
                    ...
                    "can_rollback": deployment.status in [
                        DeploymentStatus.SUCCESS,
                        DeploymentStatus.PARTIALLY_FAILED
                    ]
                })
