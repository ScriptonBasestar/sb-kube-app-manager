...
from sbkube.commands.fix import cmd, _show_fix_plan, _show_fix_summary, _show_fix_history
...
class TestFixCommand:
    ...
    def test_fix_command_basic(self, mock_create_task, mock_diagnostic_engine, mock_fix_engine, runner):
        """기본 fix 명령어 테스트"""
        # Mock 설정
    ...
    @patch('sbkube.commands.fix.DiagnosticEngine') 
    @patch('asyncio.create_task')
    def test_fix_command_dry_run(self, mock_create_task, mock_diagnostic_engine, mock_fix_engine, runner, sample_diagnostic_results):
        """Dry run 테스트"""
        # Mock 설정
        mock_diagnostic_instance = Mock()
        mock_fix_instance = Mock()
        ...
        mock_fix_instance.find_applicable_fixes.return_value = [
            (Mock(description="Test Fix", risk_level="low"), sample_diagnostic_results[0])
        ]
    ...
    @patch('sbkube.commands.fix.DiagnosticEngine')
    @patch('asyncio.create_task')
    def test_fix_command_force_mode(self, mock_create_task, mock_diagnostic_engine, mock_fix_engine, runner, sample_diagnostic_results):
        """Force 모드 테스트"""
        # Mock 설정
        mock_diagnostic_instance = Mock()
        mock_fix_instance = Mock()
...
class TestFixCommandError:
    ...
    @patch('sbkube.commands.fix.DiagnosticEngine')
    @patch('asyncio.create_task')
    def test_fix_command_with_failed_attempts(self, mock_create_task, mock_diagnostic_engine, mock_fix_engine, runner, sample_diagnostic_results):
        """실패한 수정이 있는 경우 테스트"""
        # Mock 설정
        mock_diagnostic_instance = Mock()
        mock_fix_instance = Mock()
...
class TestFixCommandIntegration:
    ...
    def test_complete_fix_workflow(
        ...
        assert len(register_calls) == 3  # MissingNamespaceFix, ConfigFileFix, HelmRepositoryFix
