...
이 예제는 `sbkube.yaml`의 `app_dirs` 필드를 사용하여 배포할 앱 그룹을 명시적으로 지정하는 방법을 보여줍니다.

## 프로젝트 구조

```
examples/app-dirs-explicit/
├── sbkube.yaml          # app_dirs 목록 포함
├── redis/sbkube.yaml     # ✅ 배포됨 (app_dirs에 포함)
├── postgres/sbkube.yaml  # ✅ 배포됨 (app_dirs에 포함)
└── nginx/sbkube.yaml     # ❌ 배포 제외 (app_dirs에 미포함)
```
...
**출력:**
```
📂 Using app_dirs from sbkube.yaml (2 group(s)):
  - postgres/
  - redis/
```
...
|------|-------------------------------|-------------------------------|
| 배포 대상 | `app_dirs`에 명시된 것만 | `sbkube.yaml` 있는 모든 디렉토리 |
| 제어 수준 | 높음 (명시적) | 낮음 (자동) |
...
```bash
# sbkube.yaml 로드 및 app_dirs 확인
yq '.app_dirs' examples/app-dirs-explicit/sbkube.yaml
```
...
**증상:**
```
❌ Invalid app_dirs in sbkube.yaml:
...
```
❌ Invalid app_dirs in sbkube.yaml:
  - Config file not found: redis/sbkube.yaml
```

**해결:**
- 해당 디렉토리에 `sbkube.yaml` 생성
- 또는 `app_dirs`에서 제거
